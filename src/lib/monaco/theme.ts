...
export const exerciseEditorOptions: editor.IStandaloneEditorConstructionOptions = {
  ...sharedOptions,
  lineNumbers: 'on',
...
export function setupPowerShellTheme(monaco: Monaco): void {
  if (!themeRegistered) {
...
export function codeBlockHeight(code: string, minLines = 2): number {
  const lines = code.split('\n').length;
  const lineHeight = 20;
  return Math.max(minLines, lines) * lineHeight + 16;
}
